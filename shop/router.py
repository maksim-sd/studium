...
from user.router import BasicAuth
...
@router.get(
    "/categories/", 
    auth=BasicAuth(), 
    response=List[CategoryProductOut], 
    summary="Категории товара", 
    tags=["Категория и товар"]
)
...
@router.get(
    ...
    auth=BasicAuth(),
    response=CategoryProductOut, 
    summary="Выбранная категория товара", 
    tags=["Категория и товар"]
)
def get_category(request, id_category:int = Path(..., description="ID категории")):
    return get_object_or_404(CategoryProduct, id=id_category)


@router.get(
    "/category/{int:id_category}/products/",
    auth=BasicAuth(), 
    response=List[ProductOut], 
    summary="Товары выбранной категории", tags=["Категория и товар"]
)
def get_category_products(request, id_category:int = Path(..., description="ID категории")):
    category = get_object_or_404(CategoryProduct, id=id_category)
    return Product.objects.filter(category_product=category)

@router.get(
    "/product_statuses/", 
    auth=BasicAuth(), 
    response=List[ClassifierOut], 
    summary="Статусы товара", 
    tags=["Категория и товар"]
)
...
@router.get(
    ...
    auth=BasicAuth(), 
    response=ProductOut, 
    summary="Выбранный товар", tags=["Категория и товар"]
)
def get_product(request, id_product:int = Path(..., description="ID товара")):
    return get_object_or_404(Product, id=id_product)
...
@router.get(
    "/cart/",
    auth=BasicAuth(),
    response=List[CartProductOut], 
    summary="Корзина текущего пользователя", tags=["Корзина"]
)
def get_cart(request):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(400, "Данный пользователь не может иметь корзину")
    cart, _ = Cart.objects.get_or_create(executor=user)
    return CartProduct.objects.filter(cart=cart)


@router.post(
    "/cart/product/{int:id_product}/",
    auth=BasicAuth(), 
    summary="Добавить выбранный товар в корзину",
    tags=["Корзина"]
)
def post_cart_product(request, id_product:int = Path(..., description="ID товара")):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
...
@router.patch(
    "/cart/product/{int:id_product}/increase/",
    auth=BasicAuth(), 
    summary="Увеличить количество выбранного товара в корзине на 1",
    tags=["Корзина"]
)
def patch_cart_product_increase(request, id_product:int = Path(..., description="ID товара")):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(403, "Недостаточно прав")
    ...
    cart_product.quantity += 1
    cart_product.save()
    return {"detail": "Количество товара в корзине увеличено на 1"}   
    

@router.patch(
    "/cart/product/{int:id_product}/decrease/",
    auth=BasicAuth(), 
    summary="Уменьшить количество выбранного товара в корзине на 1",
    tags=["Корзина"]
)
def patch_cart_product_decrease(request, id_product:int = Path(..., description="ID товара")):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(403, "Недостаточно прав")
    cart = get_object_or_404(Cart, executor=user)
    ...
    cart_product.save()
    return {"detail": "Количество товара в корзине уменьшено на 1"}       


@router.delete(
    "/cart/product/{int:id_product}/",
    auth=BasicAuth(), 
    summary="Удалить выбранный товар из корзины",
    tags=["Корзина"]
)
def delete_cart_product(request, id_product:int = Path(..., description="ID товара")):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(403, "Данный пользователь не может удалять товары из корзины")
    cart = get_object_or_404(Cart, executor=user)
    product = get_object_or_404(Product, id=id_product)
...
def get_orders(request):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(400, "Данный пользователь не может иметь заказы")
...
@router.get(
    ...
    summary="Детали выбранного заказа текущего пользователя",
    tags=["Заказ"]
)
def get_order(request, id_order:int = Path(..., description="ID заказа")):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(400, "Данный пользователь не может иметь заказы")
    order = get_object_or_404(Order, id=id_order, executor=user)
...
def post_order(request, payload:CartProductIDIn):
    user = request.auth
    if not user.groups.filter(name="Исполнитель").exists():
        raise HttpError(400, "Данный пользователь не может оформлять заказы")
    cart = get_object_or_404(Cart, executor=user)
    ...
    with transaction.atomic():
        ...
        for cart_product in cart_products:
            if cart_product.quantity > cart_product.product.stock:
                raise HttpError(400, f"Указанное количество товара '{cart_product.product.name}' нет в наличии")
            elif cart_product.product.product_status != "AVAILABLE":
                raise HttpError(400, f"Указанный товар '{cart_product.product.name}' не доступен для продажи")
