...
from profile.router import BasicAuth
from profile.models import Balance
from .models import CategoryProduct, Cart, CartProduct, Product, Order, OrderProduct
from .schemas import CategoryProductOut, ClassifierOut, ProductOut, CartProductOut, OrderOut, OrderProductOut, CartProductIDIn
...
def patch_cart_product_decrease(request, id:int):
    ...
    if cart_product.quantity <= 0:
        cart_product.delete()
        return {"Недостаточно прав": "Товар удален из корзины"} 
    cart_product.save()
...
def patch_order_status(request, id_order:int, id_status:str):
    ...
    return {"detail": "Статус заказа изменен на"}
